        ...
        public static async Task<List<DecodeError>> Encode(VideoElement video, int targetColorPrimary, int targetTransferFunction, int targetColorMatrix, string targetCreatorId)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    int itteration = 0;
                    while (File.Exists(directory + Path.DirectorySeparatorChar + videofile.Name + "_modified" + itteration.ToString() + videofile.Extension))
                        itteration++;
                    File.Copy(video.FilePath, directory + Path.DirectorySeparatorChar + videofile.Name + "_modified" + itteration.ToString() + videofile.Extension);
                    stream = new FileStream(directory + Path.DirectorySeparatorChar + videofile.Name + "_modified" + itteration.ToString() + videofile.Extension, FileMode.Open, FileAccess.ReadWrite);
                }
            }
            catch (Exception ex)
            {
